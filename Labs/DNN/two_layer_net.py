...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # pass
    z1 = X.mm(W1) + b1
    # 2. Apply ReLU activation manually (no torch.relu allowed)
    hidden = torch.clamp(z1, min=0)
    # 3. Compute scores from hidden layer: hidden @ W2 + b2
...
def nn_forward_backward(
    ...
    dscores = probs.clone()
    dscores[range(N), y] -= 1
    dscores /= N  # normalize by batch size
